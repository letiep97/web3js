...
function App() {
  ...
  const user1 = {
    privateKey:
      "383fb5b9fbdcd79b6bce0f03a67dc1c2d2f9ad315f2b6d004409ca1377591af4",
  ...
  useEffect(() => {
    (async () => {
      ...
      // Case 3:
      web3 = new Web3(
        "https://rinkeby.infura.io/v3/c9f9eba874a24d339db4c886f6964321"
      );
  ...
  const sentToken = async (address, amount) => {
    ...
    try {
      ...
      const sendAmount = new BigNumber(amount).multipliedBy(
        Math.pow(10, decimalToken)
      );
            ...
            <Form.Group className="mb-3">
              <FloatingLabel controlId="floatingSelect" label="Token">
                ...
                >
                  <option value={""}></option>
                  {listToken.map((v) => (
                    <option value={v} key={v}>SUSHI</option>
                  ))}
                </Form.Select>
              </FloatingLabel>
            </Form.Group>
